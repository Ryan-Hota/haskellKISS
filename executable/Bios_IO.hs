...
import Directory_IO (mkLinkAt, fileTreeAlong, listPermittedDirectory, doesFileExist, removePathForcibly)
...
clean :: Target -> IO ()
clean = let toDelete = ( `notElem` ["hie.yaml",".hlint.yaml"] ) in
    ( rootOf.targetPath )
    |> ( (<$>) . (<$>) . (</>) <*> listPermittedDirectory )
    >=> filterM doesFileExist
    |> fmap ( filter (takeName|>toDelete) )
